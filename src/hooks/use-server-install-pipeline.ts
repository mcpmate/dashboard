...
import { serversApi, serializeMetaForApi } from "../lib/api";
...
export function useServerInstallPipeline(
            ...
            servers: items.map((item) => ({
                ...
                command:
                    item.kind === "stdio"
                        ? item.command ?? null
                        : null,
                args: item.args && item.args.length ? item.args : null,
                    ...
                    item.kind !== "stdio"
                        ? (() => {
                              ...
                              if (!item.urlParams || !Object.keys(item.urlParams).length)
                                  return item.url;
                              try {
                                  const u = new URL(item.url, /^https?:/i.test(item.url) ? undefined : "http://dummy.local");
                        ...
                        : null,
                headers:
                    item.kind !== "stdio" && item.headers && Object.keys(item.headers).length
                        ? item.headers
                        : null,
            })),
        };
            ...
            if (item.kind !== "stdio" && item.url) {
                ...
                if (item.urlParams && Object.keys(item.urlParams).length) {
                    try {
                        ...
                        const qs = new URLSearchParams(item.urlParams as Record<string, string>).toString();
	...
	const begin = useCallback(
		async (items: ServerInstallDraft[], origin: InstallSource) => {
			if (!items.length) {
				notifyError("No servers detected", "Provide at least one server to preview");
				return;
			}
		...
		try {
			...
			const didSucceed =
				typeof result?.success === "boolean"
					? result.success
					: (result as { status?: string })?.status === "success" || !("error" in (result ?? {}));
			if (didSucceed) {
	...
	const state = useMemo(
		...
		[drafts, source, previewState, previewError, isPreviewLoading, isImporting, isDrawerOpen],
	);
