...
import InspectorDrawer, { InspectorLogEntry } from "../../components/inspector-drawer";
...
const createDebugState = (): DebugState => ({ items: [], loading: false, error: null, fetched: false });
...
export function ServerDetailPage() {
  ...
  const [requestedChannel, setRequestedChannel] = useState<InspectorChannel>(initialChannel);
  ...
  const toggleServerM = useMutation({
    mutationFn: async (enable: boolean) => {
      ...
      return enable ? serversApi.enableServer(serverId) : serversApi.disableServer(serverId);
    },
    onSuccess: (_, enable) => {
  ...
  const activeProfilesQ = useQuery({
    ...
    enabled: !!serverId && viewMode === VIEW_MODES.debug && requestedChannel === "proxy",
    queryFn: async () => {
      ...
      await Promise.all(
        active.map(async (suit) => {
          try {
            ...
            const match = (res.servers || []).find((srv) => srv.id === serverId && srv.enabled);
  ...
  const rawProxyAvailable = (activeProfilesQ.data?.enabledNames?.length ?? 0) > 0;
  const isProxyChecking =
    viewMode === VIEW_MODES.debug && activeProfilesQ.isFetching && !activeProfilesQ.isFetched;
  ...
  const updateDebugState = useCallback((kind: DebugKind, patch: Partial<DebugState>) => {
    setDebugData((prev) => ({
      ...prev,
      [kind]: {
        ...prev[kind],
  ...
  const runList = useCallback(
    async (kind: DebugKind) => {
      ...
      if (channel === "proxy" && !proxyAvailable) {
        updateDebugState(kind, {
          error: "Proxy mode unavailable: server not enabled in any active profile.",
        });
        return;
      }
      const method = kind === "tools" ? "tools/list" : kind === "resources" ? "resources/list" : "prompts/list";
    ...
    [channel, proxyAvailable, pushLog, server?.name, serverId, updateDebugState],
  );
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
        <div className="flex items-center gap-3">
          <h2 className="text-3xl font-bold tracking-tight">{serverDisplayName}</h2>
          {server ? (
            <StatusBadge
      ...
      {server && (
        <Tabs key={viewMode} defaultValue={defaultTab} className="space-y-4">
          <div className="flex items-center justify-between gap-2 flex-wrap">
            <ServerCapabilityTabsHeader serverId={serverId} viewMode={viewMode} />
          ...
          <TabsContent value="overview">
            {isLoading ? (
              ...
              <div className="grid gap-4">
                      ...
                      <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                        <div className="flex flex-wrap items-start gap-4">
                          <Avatar className="h-20 w-20 text-xl">
                            <AvatarFallback>{serverDisplayName.slice(0, 1).toUpperCase()}</AvatarFallback>
                          </Avatar>
                          <div className="grid grid-cols-[auto_1fr] gap-x-5 gap-y-2 text-sm">
                            ...
                            <span className="text-xs uppercase text-slate-500">Runtime</span>
                            <StatusBadge
                              status={runtimeStatus}
                              instances={server.instances || []}
                              isServerEnabled={serverEnabled}
                            ...
                            <span className="font-mono text-sm leading-tight">{server.kind}</span>
                            {server.command ? (
                              <>
                                <span className="text-xs uppercase text-slate-500">Command</span>
                                <span className="font-mono text-xs md:text-sm break-all">{server.command}</span>
                              </>
                            ) : null}
                            <span className="text-xs uppercase text-slate-500">Repository</span>
                ...
                <Card>
                  <CardHeader>
                    <CardTitle>Instances ({server.instances?.length || 0})</CardTitle>
                  </CardHeader>
                  <CardContent>
              ...
              <Card>
                ...
                <CardContent>
                  <p className="text-sm text-slate-600 dark:text-slate-300">
                    MCP resource templates are not yet available through the Inspector debug channel.
                  </p>
                </CardContent>
              </Card>
            )}
...
function ServerCapabilityTabsHeader({ serverId, viewMode }: { serverId: string; viewMode: keyof typeof VIEW_MODES }) {
  ...
  return (
    <TabsList className="flex flex-wrap gap-2">
      {viewMode === VIEW_MODES.browse ? <TabsTrigger value="overview">Overview</TabsTrigger> : null}
      <TabsTrigger value="tools" disabled={disableEmpty && toolsCount === 0}>
        Tools ({toolsCount})
      </TabsTrigger>
      <TabsTrigger value="resources" disabled={disableEmpty && resourcesCount === 0}>
        Resources ({resourcesCount})
      </TabsTrigger>
      <TabsTrigger value="prompts" disabled={disableEmpty && promptsCount === 0}>
        Prompts ({promptsCount})
      </TabsTrigger>
      <TabsTrigger value="templates" disabled={disableEmpty && templatesCount === 0}>
    ...
    <div ref={containerRef} className="relative inline-flex items-center gap-2">
      ...
      <div className="inline-flex rounded-md border bg-white dark:bg-slate-900">
        <Button
          ...
          onClick={handleProxy}
        >
          {isChecking ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <ShieldAlert className="h-4 w-4" />}
          Proxy
        </Button>
        <Button
          type="button"
      ...
      {!proxyAvailable && !isChecking && hintVisible ? (
        <div className="absolute right-0 top-full mt-1 w-64 rounded-md border bg-white dark:bg-slate-900 p-3 text-xs shadow-lg z-10">
          <p className="font-medium text-slate-700 dark:text-slate-200 mb-1">Proxy unavailable</p>
          <p className="text-slate-600 dark:text-slate-300 mb-2">
            Enable this server in an active profile to exercise proxy mode.
          </p>
          <Button size="sm" variant="outline" className="text-xs" onClick={onOpenProfiles}>
            Open Profiles
          </Button>
        </div>
      ) : null}
...
function InspectorDebugSection({ kind, state, disabled, onFetch, onInspect, logs, onClearLogs }: InspectorDebugSectionProps) {
  ...
  const sectionLogs = useMemo(() => {
    const prefix = kind === "tools" ? "tools/" : kind === "resources" ? "resources/" : "prompts/";
    return logs.filter((entry) => entry.method.startsWith(prefix));
  }, [logs, kind]);

  return (
    <Tabs value={tab} onValueChange={(v) => setTab(v as "results" | "logs")} className="w-full space-y-3">
      <div className="flex items-center justify-between gap-2 flex-wrap">
        <TabsList className="flex flex-wrap gap-2">
          <TabsTrigger value="results">Results ({state.items.length})</TabsTrigger>
          <TabsTrigger value="logs">Inspector Logs ({sectionLogs.length})</TabsTrigger>
        </TabsList>
        {tab === "results" ? (
          ...
          <div className="flex items-center gap-2">
            <Button size="sm" variant="outline" className="h-8" onClick={onClearLogs} disabled={!sectionLogs.length}>
              Clear Logs
            </Button>
          </div>
        )}
        ...
        <Card className="min-h-[220px]">
          <CardHeader className="px-4 pt-4 pb-0">
            {state.lastFetched ? (
              <p className="text-xs text-slate-500">
                Last listed at {new Date(state.lastFetched).toLocaleTimeString()}
              </p>
            ) : null}
            {state.error ? <p className="text-xs text-red-500 mt-1">{state.error}</p> : null}
          </CardHeader>
          <CardContent className="p-4">
            <CapabilityList
              ...
              filterText={filter}
              emptyText={state.fetched ? `No ${title.toLowerCase()} returned.` : `Run ${title.toLowerCase()} list to fetch live data.`}
              renderAction={(m, item) => (
                <Button
                  type="button"
                  size="sm"
          ...
          <CardContent className="space-y-2 p-4 max-h-[60vh] overflow-auto">
            {sectionLogs.length === 0 ? (
              ...
              sectionLogs.map((entry) => (
                <div key={entry.id} className="border rounded-md p-2 bg-slate-50 dark:bg-slate-900 text-xs space-y-1">
                  <div className="flex items-center justify-between gap-2">
                    <span className="font-mono text-[11px] text-slate-500">{new Date(entry.timestamp).toLocaleTimeString()}</span>
                    <span className="font-mono text-[11px] text-slate-500">{entry.method}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <Badge variant={entry.event === "error" ? "destructive" : entry.event === "success" ? "success" : "secondary"} className="text-[10px] uppercase">
                      {entry.event}
                    </Badge>
                    <Badge variant="outline" className="text-[10px] uppercase">{entry.mode}</Badge>
                  </div>
                  {entry.message ? <p className="text-red-500">{entry.message}</p> : null}
