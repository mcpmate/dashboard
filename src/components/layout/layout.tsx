    ...
    const handler = () => {
      try {
        ...
        if (w < 1200) {
          ...
          try { localStorage.setItem(autoKey, "1"); } catch { /* noop */ }
          return;
        }
        // Large screens: only auto-open if last close was auto
        let wasAuto = false;
        try { wasAuto = localStorage.getItem(autoKey) === "1"; } catch { /* noop */ }
        if (w >= 1200 && wasAuto && !sidebarOpen) {
          setSidebarOpen(true);
          try { localStorage.removeItem(autoKey); } catch { /* noop */ }
        }
      } catch { /* noop */ }
    };
    handler();
      ...
      >
        ...
        <div className="container mx-auto p-4 min-h-[calc(100vh-4rem)] flex flex-col">
          ...
          <footer className="mt-6 text-[11px] text-slate-500 border-t pt-2 pb-1 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
            <div>
              MCPMate Dashboard <span className="font-mono">v0.1.0</span> •
              {" "}
              <a className="hover:underline" href="https://mcpmate.io" target="_blank" rel="noreferrer">© 2025 MCPMate</a>
            </div>
            <div className="flex items-center gap-3">
              <a className="hover:underline" href="http://127.0.0.1:8080/docs" target="_blank" rel="noreferrer">API Docs</a>
              <a className="hover:underline" href="https://www.modelcontextprotocol.io" target="_blank" rel="noreferrer">Model Context Protocol</a>
            </div>
          </footer>
        </div>
      </main>
