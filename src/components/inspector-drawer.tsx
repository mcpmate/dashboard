...
export function InspectorDrawer({ open, onOpenChange, serverId, serverName, kind, item, mode, onLog }: InspectorDrawerProps) {
  ...
  const [uri, setUri] = useState<string>(String(item?.resource_uri || item?.uri || ""));
  const [name, setName] = useState<string>(String(item?.unique_name || item?.tool_name || item?.prompt_name || item?.name || ""));
      ...
      if (kind === 'tool') {
        ...
        if (props && Object.keys(props).length > 0) {
          ...
          list = Object.keys(props).map((k) => {
            const p = props[k] || {};
            const type = Array.isArray(p.type) ? String(p.type[0]) : String(p.type || 'string');
            const en = Array.isArray(p.enum) ? (p.enum as string[]) : undefined;
            return { name: k, type, required: required.includes(k), description: p.description, enum: en };
          });
        }
  ...
  useEffect(() => {
    ...
    if (schema && schema.type === 'object' && schema.properties && Object.keys(schema.properties).length > 0) {
      ...
      if (fs.length > 0) {
        ...
        const genSchema = { type: 'object', properties: genProps, required: reqNames } as any;
  ...
  return (
    <Drawer open={open} onOpenChange={onOpenChange}>
      <DrawerContent>
        <DrawerHeader>
          <DrawerTitle>Inspector · {kind === "tool" ? "Tool Call" : kind === "resource" ? "Read Resource" : "Get Prompt"}</DrawerTitle>
          <DrawerDescription>Run quick calls against server capabilities without leaving the page.</DrawerDescription>
        </DrawerHeader>
        ...
        <div className="px-4 py-3 space-y-3">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            ...
            {kind === "tool" ? (
              <div className="space-y-1">
                ...
                <Input type="number" min={1000} step={500} value={timeoutMs} onChange={(e) => setTimeoutMs(parseInt(e.target.value) || 8000)} />
              </div>
            ) : null}
            <div className="space-y-1">
              <Label>Server</Label>
                ...
                <div className="flex items-center gap-2 text-xs">
                  <Button size="sm" variant="outline" onClick={() => {
                    if (schemaObj) {
                      ...
                      setValues(mock); setArgsJson(JSON.stringify(mock, null, 2));
                    } else {
                      const mock = fillMock(fields); setValues(mock); setArgsJson(JSON.stringify(mock, null, 2));
                    }
                  }}>Fill Mock</Button>
                  <Button size="sm" variant="outline" onClick={() => { setValues({}); setArgsJson("{}"); }}>Clean</Button>
                  <Button size="sm" variant={useRaw ? "default" : "outline"} onClick={() => setUseRaw((v) => !v)}>{useRaw ? "Form" : "JSON"}</Button>
                </div>
              </div>
              {useRaw ? (
                <Textarea rows={8} className="font-mono text-xs" value={argsJson} onChange={(e) => setArgsJson(e.target.value)} />
              ) : schemaObj ? (
                <SchemaForm schema={schemaObj} value={values} onChange={(v)=>{ setValues(v); setArgsJson(JSON.stringify(v, null, 2)); }} />
              ) : (
                <Textarea rows={6} className="font-mono text-xs" value={argsJson} onChange={(e) => setArgsJson(e.target.value)} />
              )}
            </div>
          ) : null}
          ...
          {result ? (
            <div className="space-y-2">
              ...
              <pre className="bg-slate-50 dark:bg-slate-900 rounded p-3 text-xs overflow-auto max-h-[40vh]">{pretty(result)}</pre>
            </div>
          ) : null}
        </div>
        ...
        <DrawerFooter>
          <div className="flex gap-2 w-full">
            ...
            <Button className="flex-1" onClick={onSubmit} disabled={submitting}>{submitting ? "Running..." : "Run"}</Button>
          </div>
        </DrawerFooter>
      </DrawerContent>
    </Drawer>
