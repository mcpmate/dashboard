...
export function ServerFormDrawer({
  ...
  const handleFormSubmit = async (data: ServerFormValues) => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Error saving server configuration",
      );
    } finally {
      setIsSubmitting(false);
    }
  ...
  // Import helpers
  function parsePreviewPayload(): { ok: boolean; payload?: any; error?: string } {
    try {
      ...
      return { ok: false, error: "JSON must include `mcpServers` mapping or `servers` array" };
    } catch (e) {
      return { ok: false, error: String(e) };
    }
  }

  function parseImportPayload(): { ok: boolean; payload?: any; error?: string } {
    try {
      const obj = JSON.parse(importText);
      if (obj.mcpServers && typeof obj.mcpServers === "object") {
        ...
        for (const key of Object.keys(obj.mcpServers)) {
          ...
          normalized[key] = { type: conf.type || conf.kind || "stdio", command: conf.command ?? null, args: Array.isArray(conf.args) ? conf.args : null, env: conf.env ?? null, url: conf.url ?? null };
        }
      ...
      if (Array.isArray(obj.servers)) {
        ...
        for (const s of obj.servers) {
          if (!s?.name) return { ok: false, error: "servers[] items must include name" };
          mapping[s.name] = {
  ...
  async function doPreview() {
    ...
    try {
      setIsSubmitting(true);
      const res = await serversApi.previewServers({ ...p.payload, timeout_ms: 4000 });
      setPreview(res);
  ...
  async function doImport() {
    ...
    try {
      ...
      notifySuccess(res.success ? "Imported" : "Import finished", res.success ? "Servers imported successfully" : "See summary below");
      onImported?.();
      onClose();
    } catch (e) {
    ...
    <Drawer open={isOpen} onOpenChange={onClose}>
      <DrawerContent>
        <DrawerHeader>
          ...
          <DrawerDescription>
            Configure server connection information. You can use a form
            or paste a JSON snippet to import servers.
          </DrawerDescription>
        </DrawerHeader>

        <div className="flex-1 overflow-y-auto px-4">
          <Tabs defaultValue="form" value={tab} onValueChange={(v) => setTab(v as any)}>
                ...
                <TabsList>
                  ...
                  <TabsTrigger value="import">
                    <span className="flex items-center gap-2">
                      Import JSON
                      <Badge variant="warning" className="h-5 px-2 py-0 text-[10px]">WIP</Badge>
                    </span>
                  </TabsTrigger>
                </TabsList>
              </div>
              ...
              <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-4 py-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    {isEditing && (
                      <p className="text-xs text-gray-500">Server name cannot be changed after creation</p>
                    )}
                  </div>

                  <div className="space-y-2">
                    <Label htmlFor="kind">Server Type</Label>
                    <Select defaultValue={defaultValues.kind} onValueChange={(value) => setValue("kind", value as any)}>
                      <SelectTrigger>
                        <SelectValue placeholder="Select server type" />
                      </SelectTrigger>
                      <SelectContent>
                        <SelectItem value="stdio">Standard Input/Output (stdio)</SelectItem>
                        <SelectItem value="sse">Server-Sent Events (SSE)</SelectItem>
                        <SelectItem value="streamable_http">HTTP Stream (Streamable HTTP)</SelectItem>
                      </SelectContent>
                    </Select>
                    {errors.kind && (
                ...
                {serverType === "stdio" && (
                  <>
                    <div className="space-y-2">
                      ...
                      <Input id="command" {...register("command")} placeholder="e.g., python -m my_script" />
                    </div>

                    <div className="space-y-2">
                      <Label htmlFor="command_path">Command Path</Label>
                      <Input id="command_path" {...register("command_path")} placeholder="e.g., /usr/local/bin" />
                    </div>

                    <div className="space-y-2">
                      <Label htmlFor="args">Arguments (space separated)</Label>
                      <Input id="args" {...register("args")} placeholder="e.g., --debug --port 8080" />
                    </div>
                  </>
                )}
                ...
                {(serverType === "sse" || serverType === "streamable_http") && (
                  <div className="space-y-2">
                    ...
                    <Input id="command" {...register("command")} placeholder="e.g., http://localhost:8080" />
                  </div>
                )}

                <div className="space-y-2">
                  <Label htmlFor="env">Environment Variables (one per line, KEY=VALUE format)</Label>
                  <Textarea id="env" {...register("env")} placeholder="e.g.,&#10;PORT=8080&#10;DEBUG=true" rows={4} />
                </div>

                <div className="space-y-2">
                  <Label htmlFor="max_instances">Maximum Instances</Label>
                  <Input id="max_instances" type="number" min="1" {...register("max_instances")} />
                </div>
              </form>
            </TabsContent>
            ...
            {enableImportTab && !isEditing ? (
              <TabsContent value="import">
                <div className="space-y-3 py-4">
                  <Textarea rows={12} value={importText} onChange={(e) => setImportText(e.target.value)} className="font-mono text-xs" />
                  <div className="flex gap-2">
                    <Button variant="outline" type="button" onClick={formatJson}>Format</Button>
                    <Button type="button" onClick={doPreview} disabled={isSubmitting}>Preview</Button>
                    <Button type="button" variant="secondary" onClick={doImport} disabled={isSubmitting}>Import</Button>
                  </div>
                  {preview && (
                    <div className="rounded border p-3">
                      ...
                      {preview.data?.items?.some((it: any) => it.error) ? (
                        <div className="mb-2 text-xs text-amber-600 dark:text-amber-400">
                          Some servers couldn’t be contacted during preview (e.g. command not installed or terminated early). You can still import the configuration; capabilities will populate once the server runs.
                        </div>
                      ) : null}
                        ...
                        <div className="space-y-2 text-sm">
                          {preview.data.items.map((it: any) => (
                            <div key={it.name} className="rounded border p-2">
                              <div className="font-medium">{it.name} {it.ok === false ? <span className="text-red-500">(error)</span> : null}</div>
                              {it.error ? <div className="text-xs text-red-500">{String(it.error)}</div> : null}
                              <div className="text-xs text-slate-500 mt-1">tools: {it.tools?.items?.length ?? 0} • resources: {it.resources?.items?.length ?? 0} • templates: {it.resource_templates?.items?.length ?? 0} • prompts: {it.prompts?.items?.length ?? 0}</div>
                            </div>
                          ))}
                        </div>
                      ) : (
        ...
        <DrawerFooter>
          <div className="flex gap-2 w-full">
            ...
            {tab === "form" ? (
              <Button type="submit" onClick={handleSubmit(handleFormSubmit)} disabled={isSubmitting} className="flex-1">
                {isSubmitting && (<Loader2 className="mr-2 h-4 w-4 animate-spin" />)}
                {submitLabel}
              </Button>
            ) : (
              <Button type="button" onClick={doImport} disabled={isSubmitting} className="flex-1">
                {isSubmitting && (<Loader2 className="mr-2 h-4 w-4 animate-spin" />)}
                Import
              </Button>
            )}
...
function sample() {
  return JSON.stringify(
    {
      mcpServers: {
        example_stdio: { type: "stdio", command: "node", args: ["server.js"], env: { NODE_ENV: "production" } },
