...
import { BellRing, CheckCircle2, Info, AlertTriangle, XCircle } from "lucide-react";
...
export function NotificationCenter() {
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button variant="ghost" size="icon" aria-label="Notifications">
          <div className="relative">
            <BellRing size={20} />
            {unreadCount > 0 ? (
              <Badge className="absolute -top-1 -right-1 h-4 min-w-4 px-1 p-0 flex items-center justify-center" variant="destructive">
      ...
      <DropdownMenuContent align="end" className="w-[360px] max-h-[60vh] overflow-auto p-0">
        <div className="px-3 py-2 flex items-center justify-between sticky top-0 bg-popover z-10 border-b">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button variant="outline" size="sm" onClick={clear} disabled={items.length === 0}>Clear</Button>
          </div>
        </div>
        {items.length === 0 ? (
            ...
            {items.map((n) => (
              <DropdownMenuItem
                ...
                onSelect={(e) => {
                  ...
                  markRead(n.id);
                  if (n.href) {
                    try { window.open(n.href, "_blank", "noopener,noreferrer"); } catch { /* noop */ }
                  }
                }}
              >
                <div className="flex w-full items-start gap-2">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between">
                      <div className={`text-sm font-medium ${n.read ? "text-slate-600" : "text-slate-900"}`}>
                        {n.title}
                      </div>
