...
import { Drawer, DrawerContent, DrawerFooter, DrawerHeader, DrawerTitle, DrawerDescription } from "./ui/drawer";
...
export function ServerImportDrawer({
  ...
  function parsePayload(): { ok: boolean; payload?: any; error?: string } {
    try {
      ...
      if (Array.isArray(obj.servers)) {
        return { ok: true, payload: { servers: obj.servers, include_details: true } };
      }
  ...
  async function doImport() {
    ...
    try {
      ...
      notifySuccess(res.success ? "Imported" : "Import finished", res.success ? "Servers imported successfully" : "See summary above");
    ...
    <Drawer open={open} onOpenChange={onOpenChange}>
      <DrawerContent>
        <DrawerHeader>
          ...
          <DrawerDescription>Paste JSON with `mcpServers` mapping or a `servers` array.</DrawerDescription>
        </DrawerHeader>
        <div className="p-4 space-y-3">
          ...
          <div className="flex gap-2">
            ...
            <Button onClick={doPreview} disabled={previewM.isPending}>Preview</Button>
            <Button variant="secondary" onClick={doImport} disabled={importing}>Import</Button>
          </div>
                  ...
                  {preview.data.items.map((it: any) => (
                    <div key={it.name} className="rounded border p-2">
                      ...
                      <div className="text-xs text-slate-500 mt-1">
                        tools: {it.tools?.items?.length ?? 0} • resources: {it.resources?.items?.length ?? 0} • templates: {it.resource_templates?.items?.length ?? 0} • prompts: {it.prompts?.items?.length ?? 0}
                      </div>
                    </div>
                  ))}
                </div>
