...
interface AppState {
	...
	setDashboardSetting: <K extends keyof DashboardSettings>(key: K, value: DashboardSettings[K]) => void;
...
function normalizeDashboardSettings(
	...
	if (patch.clientDefaultMode === "hosted" || patch.clientDefaultMode === "transparent") {
	...
	if (Array.isArray(patch.marketBlacklist)) {
		...
		next.marketBlacklist = Array.from(unique.values()).sort((a, b) => b.hiddenAt - a.hiddenAt);
	}
...
function readDashboardSettings(): DashboardSettings {
	...
	try {
		...
		return normalizeDashboardSettings(defaultDashboardSettings, parsed ?? undefined);
	} catch {
		return { ...defaultDashboardSettings };
	}
}
...
export const useAppStore = create<AppState>((set) => ({
  ...
  setDashboardSetting: (key, value) => {
    set((state) => {
      const next = normalizeDashboardSettings(state.dashboardSettings, { [key]: value } as Partial<DashboardSettings>);
  ...
  removeFromMarketBlacklist: (serverId) => {
    set((state) => {
      const filtered = state.dashboardSettings.marketBlacklist.filter((entry) => entry.serverId !== serverId);
      const next = normalizeDashboardSettings(state.dashboardSettings, { marketBlacklist: filtered });
      persistDashboardSettings(next);
      return { dashboardSettings: next };
    });
  },
	addToMarketBlacklist: (entry) => {
		set((state) => {
			const existing = state.dashboardSettings.marketBlacklist.filter((item) => item.serverId !== entry.serverId);
			const updated = [...existing, entry];
			const next = normalizeDashboardSettings(state.dashboardSettings, { marketBlacklist: updated });
			persistDashboardSettings(next);
			return { dashboardSettings: next };
		});
	},
