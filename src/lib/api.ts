import type {
	...
	CreateConfigSuitRequest, InstallResponse,
...
function extractApiData<T>(response: ApiWrapper<T>): T {
	...
	throw new Error(response.error ? String(response.error) : "API request failed");
}
...
export const serversApi = {
	getAll: async (): Promise<ServerListResponse> => {
		try {
			...
			return { servers: Array.isArray(resp?.data?.servers) ? resp.data.servers : [] };
		} catch (error) {
	...
	getServer: async (id: string): Promise<ServerDetail> => {
		try {
			...
			const resp = await fetchApi<ServerDetailsResp>(`/api/mcp/servers/details?${q}`);
	...
	getInstances: async (serverId: string) => {
		...
		const resp = await fetchApi<InstanceListResp>(`/api/mcp/servers/instances/list?${q}`);
	...
	getInstance: async (serverId: string, instanceId: string): Promise<InstanceDetail> => {
		try {
			...
			const resp = await fetchApi<InstanceDetailsResp>(`/api/mcp/servers/instances/details?${q}`);
			const data = resp?.data;
			return {
	...
	getInstanceHealth: async (serverId: string, instanceId: string) => {
		const q = new URLSearchParams({ server: serverId, instance: instanceId });
		const resp = await fetchApi<InstanceHealthResp>(`/api/mcp/servers/instances/health?${q}`);
		return (resp?.data as InstanceHealth) ?? {
			id: instanceId,
			name: instanceId,
			healthy: false,
			message: "No data",
...
export const toolsApi = {
	...
	getSuits: async () => {
		try {
			return await fetchApi<{ suits: { id: string; name: string }[] }>("/api/mcp/suits/list");
		} catch (error) {
	...
	getSuitTools: async (suitId: string) => {
		try {
			return await fetchApi<{ tools: SuitTool[] }>(`/api/mcp/suits/tools/list?suit_id=${suitId}`);
		} catch (error) {
	...
	_manageTool: async (suitId: string, suitToolId: string, action: string) => {
		try {
			...
			console.warn(`${action} tool API not available, using mock implementation:`, error);
			return {
				status: "success",
...
export const runtimeApi = {
	getStatus: async (): Promise<RuntimeStatusResponse> => {
		const response = await fetchApi<ApiWrapper<RuntimeStatusResponse>>("/api/runtime/status");
		return extractApiData(response);
	},

	getCache: async (): Promise<RuntimeCacheResponse> => {
		const response = await fetchApi<ApiWrapper<RuntimeCacheResponse>>("/api/runtime/cache");
		return extractApiData(response);
	},

	resetCache: (cacheType?: "all" | "uv" | "bun") =>
		fetchApi<ClearCacheResponse>(
			cacheType ? `/api/runtime/cache/reset?cache_type=${cacheType}` : "/api/runtime/cache/reset",
			{ method: "POST" }
		),
...
export const capabilitiesApi = {
	getStats: async (): Promise<CapabilitiesStatsResponse> => {
		const response = await fetchApi<ApiWrapper<{
	...
	getKeys: (params?: { limit?: number; offset?: number; search?: string }) => {
		...
		return fetchApi<CapabilitiesKeysResponse>(`/api/cache/capabilities/details?${q}`);
	},

	reset: () =>
		fetchApi<ClearCacheResponse>("/api/cache/capabilities/reset", { method: "POST" }),
};
...
export const configApi = {
	getCurrentConfig: async (): Promise<MCPConfig> => {
		try {
			...
			console.warn("Config API not available, building from available data:", error);
			...
			try {
				...
				if (Array.isArray(serversResponse?.servers)) {
					config.servers = serversResponse.servers.map(server => ({
						name: server.name,
						kind: (server.kind === "stdio" || server.kind === "sse") ? server.kind : "streamable_http",
						command: "",
	...
	deletePreset: (id: string) =>
		fetchApi<ApiResponse<null>>(`/api/config/presets/${id}`, { method: "DELETE" }),

	applyPreset: (id: string) =>
		fetchApi<ApiResponse<null>>(`/api/config/presets/${id}/apply`, { method: "POST" }),
};
...
export const configSuitsApi = {
	getAll: async (): Promise<ConfigSuitListResponse> => {
		try {
			const response = await fetchApi<ApiWrapper<{ suits: ConfigSuit[]; total: number; timestamp: string }>>(
				"/api/mcp/suits/list"
			);
			const data = extractApiData(response);
	...
	getSuit: async (id: string): Promise<ConfigSuit> => {
		const q = new URLSearchParams({ id });
		const response = await fetchApi<ApiWrapper<ConfigSuit>>(`/api/mcp/suits/details?${q}`);
		return extractApiData(response);
	},

	createSuit: async (data: CreateConfigSuitRequest): Promise<ApiResponse<ConfigSuit>> => {
		const response = await fetchApi<ApiWrapper<ConfigSuit>>("/api/mcp/suits/create", {
			method: "POST",
	...
	updateSuit: async (id: string, data: UpdateConfigSuitRequest): Promise<ApiResponse<ConfigSuit>> => {
		const response = await fetchApi<ApiWrapper<ConfigSuit>>("/api/mcp/suits/update", {
			method: "POST",
	...
	deleteSuit: async (id: string): Promise<ApiResponse<null>> => {
		const response = await fetchApi<ApiWrapper<unknown>>("/api/mcp/suits/delete", {
			method: "DELETE",
	...
	_manageSuit: async (id: string, action: string) => {
		const response = await fetchApi<ApiWrapper<unknown>>("/api/mcp/suits/manage", {
			method: "POST",
	...
	// Batch operations
	batchActivate: (ids: string[]) => executeBatchOperation(ids, configSuitsApi.activateSuit),
	batchDeactivate: (ids: string[]) => executeBatchOperation(ids, configSuitsApi.deactivateSuit),
	...
	getServers: async (suitId: string): Promise<ConfigSuitServersResponse> => {
		...
		const response = await fetchApi<ApiWrapper<{ suit_id: string; suit_name: string; servers: ConfigSuitServer[] }>>(
	...
	getTools: async (suitId: string): Promise<ConfigSuitToolsResponse> => {
		const q = new URLSearchParams({ suit_id: suitId });
		const response = await fetchApi<ApiWrapper<{ suit_id: string; suit_name: string; tools: ConfigSuitTool[] }>>(
			`/api/mcp/suits/tools/list?${q}`
		);
		return extractApiData(response);
	},
	...
	getResources: async (suitId: string): Promise<ConfigSuitResourcesResponse> => {
		try {
			return await fetchApi<ConfigSuitResourcesResponse>(`/api/mcp/suits/resources/list?suit_id=${suitId}`);
		} catch (error) {
			console.warn(`Resources endpoint not implemented yet for config suit ${suitId}:`, error);
	...
	getPrompts: async (suitId: string): Promise<ConfigSuitPromptsResponse> => {
		try {
			return await fetchApi<ConfigSuitPromptsResponse>(`/api/mcp/suits/prompts/list?suit_id=${suitId}`);
		} catch (error) {
			console.warn(`Prompts endpoint not implemented yet for config suit ${suitId}:`, error);
			return { suit_id: suitId, suit_name: "Unknown", prompts: [] };
		}
	},

	// Component management operations
	_manageComponent: (endpoint: string, suitId: string, componentId: string, action: string) =>
